...
static void fd_constructor(const lv_obj_class_t *cls, lv_obj_t *obj)
{
    ...
    f->prev = NULL;
    f->next = synthui_fader_list;
    if (f->next) f->next->prev = f;
    synthui_fader_list = f;
...
static bool fd_cap_extent(const synthui_fader_t *f, float value, lv_area_t *a)
{
    ...
    if (!synthui_fader_geom(f, &g, &coords)) return false;
    const float cy = synthui_fader_cap_y(&g, value);
 ...
 * second engine can share it (the rotary's seam): fd_palette stays the pure
 * state->colors core, exported via synthui_fader_private.h for the GPU
 * TU as synthui_fader_palette() below ---- */
...
static void fd_input_pressing(lv_event_t *e)
{
    ...
    lv_indev_get_point(indev, &p);
    lv_area_t coords; synthui_fader_geom_t g;
    if (!synthui_fader_geom(f, &g, &coords)) return;
...
static void fd_draw(synthui_fader_t *f, lv_layer_t *layer)
{
    ...
    if (!synthui_fader_geom(f, &g, &c)) return;
    const float u = g.u;
    ...
    /* cap */
    {
        const float cy = synthui_fader_cap_y(&g, f->value);
